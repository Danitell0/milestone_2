...
from .maze_display import COLOR_CHOICES, CHAR_THEMES, DEFAULT_COLORS, DEFAULT_CHARS, convert_path_from_letters, render
...
class Menu(object):
    def __init__(self, title: str,
              ...
              stdscreen: curses.window, background=None):
        self.title = title
...
class MazeWindow(object):
    ...
    def build_maze(self, settings: MazeConfig):
        ...
        self.letters = maze_solver(maze, settings.entry_point, settings.exit_point)
    ...
    def paint(self, win, rows, top=0) -> None:
        ...
        for y, segments in enumerate(rows):
            ...
            for text, color_name in segments:
                if x >= max_x - 1:
                    break
                attr = self.pairs.get(color_name, curses.A_NORMAL)
                try:
